    ...
    <div style={{ display:'flex', height:'100vh', overflow:'hidden' }}>
      ...
      <aside style={{ width:220, minWidth:220, background:'var(--surface)', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', padding:'20px 12px', gap:2, overflow:'hidden auto' }}>
        ...
        {NAV.map((item, i) => {
          ...
          return (
            <div key={item.path} onClick={()=>router.push(item.path)} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 10px', borderRadius:'var(--r-sm)', cursor:'pointer', fontSize:'.83rem', fontWeight:500, color:active?'var(--purple2)':'var(--muted2)', background:active?'rgba(124,92,252,.14)':'transparent', borderLeft:active?'3px solid var(--purple)':'3px solid transparent', transition:'all .18s' }}
        ...
        <div style={{ marginTop:'auto', paddingTop:16, borderTop:'1px solid var(--border)', display:'flex', alignItems:'center', gap:8 }}>
          <Avatar name={session?.user?.name||'You'} url={session?.user?.image} size={32} color="#7C5CFC"/>
