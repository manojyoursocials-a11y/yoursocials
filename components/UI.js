...
export function Btn({ children, variant = 'primary', size = 'md', onClick, disabled, style, type = 'button' }) {
  const sz = size === 'sm' ? { padding: '5px 12px', fontSize: '.75rem' }
           : size === 'lg' ? { padding: '12px 24px', fontSize: '.95rem' }
                           : { padding: '8px 16px',  fontSize: '.82rem' };
  const vr = variant === 'primary' ? { background: 'var(--grad1)', color: '#fff' }
  ...
  return (
    <button
      ...
      style={{ border: 'none', cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'Inter,sans-serif', fontWeight: 600, borderRadius: 10, transition: 'all .18s', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, opacity: disabled ? .5 : 1, ...sz, ...vr, ...style }}
    >
      {children}
    </button>
  );
...
export function Card({ children, style, hover, onClick }) {
  return (
    <div
      onClick={onClick}
      style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 20, transition: hover ? 'transform .18s,box-shadow .18s' : undefined, cursor: onClick ? 'pointer' : undefined, ...style }}
...
export function Modal({ open, onClose, title, children, width = 560 }) {
  ...
  return (
    <div
      ...
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', backdropFilter: 'blur(6px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
    >
...
const iStyle = {
  ...
  borderRadius: 10, padding: '9px 12px', fontSize: '.83rem', color: 'var(--text)',
  fontFamily: 'Inter,sans-serif', outline: 'none', transition: 'border .18s',
};
...
export function Input({ label, ...p }) {
  return <div style={{ marginBottom: 14 }}>{label && <Lbl t={label} />}<input style={iStyle} onFocus={onF} onBlur={onB} {...p} /></div>;
}
export function Select({ label, children, ...p }) {
  return <div style={{ marginBottom: 14 }}>{label && <Lbl t={label} />}<select style={{ ...iStyle, cursor: 'pointer' }} onFocus={onF} onBlur={onB} {...p}>{children}</select></div>;
}
export function Textarea({ label, ...p }) {
  return <div style={{ marginBottom: 14 }}>{label && <Lbl t={label} />}<textarea style={{ ...iStyle, resize: 'vertical', minHeight: 80 }} onFocus={onF} onBlur={onB} {...p} /></div>;
}

/* ── Tag ───────────────────────────────────────────── */
const PS = {
  P1: { background: 'rgba(255,77,109,.15)',  color: 'var(--red)' },
  P2: { background: 'rgba(255,140,66,.15)',  color: 'var(--orange)' },
  P3: { background: 'rgba(255,214,10,.12)',  color: 'var(--yellow)' },
  P4: { background: 'rgba(0,229,160,.10)',   color: 'var(--green)' },
  todo:        { background: 'rgba(144,144,170,.12)', color: 'var(--muted2)' },
  inprogress:  { background: 'rgba(0,212,255,.12)',   color: 'var(--cyan)' },
  review:      { background: 'rgba(255,214,10,.12)',  color: 'var(--yellow)' },
  done:        { background: 'rgba(0,229,160,.10)',   color: 'var(--green)' },
};
export function Tag({ text, style }) {
  const s = PS[text] || {};
  return <span style={{ display: 'inline-flex', alignItems: 'center', padding: '2px 8px', borderRadius: 6, fontSize: '.68rem', fontWeight: 700, whiteSpace: 'nowrap', ...s, ...style }}>{text}</span>;
}
...
export function Avatar({ name = '', url, size = 36, color = 'var(--purple)' }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: url ? 'transparent' : `${color}33`, border: `2px solid ${color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * .35, fontWeight: 800, color, flexShrink: 0, overflow: 'hidden' }}>
...
export function ProgressBar({ value = 0, max = 100, color = 'var(--grad1)', height = 6, style }) {
  ...
  return (
    <div style={{ height, background: 'var(--surface3)', borderRadius: height, overflow: 'hidden', ...style }}>
    ...
    <div style={{ position: 'relative', width: size, height: size, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)', position: 'absolute' }}>
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={color} strokeWidth={strokeWidth}
          strokeDasharray={c} strokeDashoffset={c - c * p} strokeLinecap="round"
...
export function TypingDots() {
  return (
    <div style={{ display: 'flex', gap: 4, padding: '4px 0' }}>
      {[0, 1, 2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--purple2)', animation: 'bounce .9s infinite', animationDelay: `${i * .15}s` }} />)}
    </div>
  );
}
...
export function ToastContainer() {
  ...
  _addToast = (msg, icon = '✅', type = 'success') => {
    const id = Date.now();
  ...
  return (
    <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 9999, display: 'flex', flexDirection: 'column', gap: 8 }}>
      {toasts.map(t => (
        <div key={t.id} style={{ background: 'var(--surface2)', border: `1px solid ${t.type === 'error' ? 'rgba(255,77,109,.3)' : 'rgba(0,229,160,.2)'}`, borderRadius: 'var(--r-sm)', padding: '12px 16px', fontSize: '.82rem', display: 'flex', alignItems: 'center', gap: 10, boxShadow: 'var(--shadow-lg)', animation: 'slideIn .3s ease', maxWidth: 320 }}>
...
export const toast = {
  success: (m, i) => _addToast?.(m, i || '✅', 'success'),
  error:   (m, i) => _addToast?.(m, i || '❌', 'error'),
  info:    (m, i) => _addToast?.(m, i || '💡', 'info'),
};
...
function tone(f, d = .2, t = 'sine', v = .12) {
...
export const sounds = {
  ...
  unlock:   () => [392, 523, 659, 784, 1047].forEach((f, i) => setTimeout(() => tone(f, .25, 'triangle', .12), i * 80)),
};
