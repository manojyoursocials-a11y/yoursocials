...
export function getDb() {
  ...
  return {
    ...
    async getReimbursements(userId, isAdmin) {
      if (isAdmin) return sql`SELECT r.*, u.name as submitter_name FROM reimbursements r LEFT JOIN users u ON r.submitted_by=u.id ORDER BY r.created_at DESC`;
      return sql`SELECT r.*, u.name as submitter_name FROM reimbursements r LEFT JOIN users u ON r.submitted_by=u.id WHERE r.submitted_by=${userId} ORDER BY r.created_at DESC`;
    },
    ...
    async getGalleryPhotos({albumId, status, all} = {}) {
      if (all) return sql`SELECT gp.*, u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id ORDER BY gp.created_at DESC`;
      if (albumId) return sql`SELECT gp.*, u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id WHERE gp.album_id=${albumId} AND gp.status='approved' ORDER BY gp.created_at DESC`;
      if (status) return sql`SELECT gp.*, u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id WHERE gp.status=${status} ORDER BY gp.created_at DESC`;
      return sql`SELECT gp.*, u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id WHERE gp.status='approved' ORDER BY gp.created_at DESC`;
    },
    async getPendingPhotos() {
      return sql`SELECT gp.*, u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id WHERE gp.status='pending' ORDER BY gp.created_at DESC`;
    },
