...
export function getDb() {
  ...
  return {
    ...
    async getTodayTasks() {
      ...
      return sql`SELECT t.*,u.name AS owner_name,c.name AS client_name FROM tasks t LEFT JOIN users u ON t.owner_id=u.id LEFT JOIN clients c ON t.client_id=c.id WHERE t.status!='done' AND (t.deadline::text=${today} OR t.post_date::text=${today}) ORDER BY t.priority`;
    },
    ...
    // ── TODAY NOTES (personal scratchpad) ──────────────────
    async getTodayNotes(userId) {
      return sql`SELECT * FROM today_notes WHERE user_id=${userId} ORDER BY created_at ASC`;
    },
