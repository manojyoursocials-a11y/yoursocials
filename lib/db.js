...
export async function setupDatabase() {
  ...
  await sql`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS quality_rating INTEGER`;
  await sql`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS actual_hours NUMERIC(5,1)`;
  await sql`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS estimated_hours NUMERIC(5,1)`;
  ...
  // Web Push subscriptions — one row per browser per user
  ...
  await sql`CREATE TABLE IF NOT EXISTS lead_interactions (
    id TEXT PRIMARY KEY,
  ...
  await sql`CREATE TABLE IF NOT EXISTS reimbursements (
    id TEXT PRIMARY KEY,
    title TEXT NOT NULL,
  ...
  await sql`CREATE TABLE IF NOT EXISTS important_days (
    id TEXT PRIMARY KEY,
    title TEXT NOT NULL,
    date DATE NOT NULL,
...
export function getDb() {
  ...
  return {
    ...
    // ── DASHBOARD ─────────────────────────────────────────
    async getDashboard() {
      // Each query wrapped individually so one failing table doesn't break dashboard
      const tasks = await sql`SELECT status,priority,deadline::text,post_date::text,title FROM tasks`.catch(()=>[]);
      const members = await sql`SELECT id,name,coins,streak,job_title,role,image FROM users ORDER BY coins DESC`.catch(()=>[]);
      const rewards = await sql`SELECT * FROM rewards ORDER BY reward_type,coin_cost`.catch(()=>[]);
      const followups = await sql`SELECT id FROM followups WHERE status='pending'`.catch(()=>[]);
      ...
      return {stats:{total,done,inProg,review,todo,overdue,completionPct:compPct,healthScore:health,pendingFollowups:followups.length,totalCoins},dueToday,members,rewards};
    },
    // ── FINANCE ─────────────────────────────────────────
    ...
    async getNotifications(userId) {
      return sql`SELECT id,user_id,type,title,read,task_id,created_at FROM notifications WHERE user_id=${userId} ORDER BY created_at DESC LIMIT 20`;
    },
    ...
    // ── PUSH SUBSCRIPTIONS ───────────────────────────
    async savePushSubscription({id, user_id, endpoint, p256dh, auth}) {
      await sql`INSERT INTO push_subscriptions(id,user_id,endpoint,p256dh,auth)
        VALUES(${id},${user_id},${endpoint},${p256dh},${auth})
        ON CONFLICT(endpoint) DO UPDATE SET user_id=${user_id}, p256dh=${p256dh}, auth=${auth}`;
    },
    async getPushSubscriptions(user_id) {
      return sql`SELECT * FROM push_subscriptions WHERE user_id=${user_id}`;
    },
    async getAllPushSubscriptions() {
      return sql`SELECT ps.*, u.name as user_name FROM push_subscriptions ps LEFT JOIN users u ON ps.user_id=u.id`;
    },
    async deletePushSubscription(endpoint) {
      await sql`DELETE FROM push_subscriptions WHERE endpoint=${endpoint}`;
    },
    async getPushSubCount() {
      const r = await sql`SELECT COUNT(*)::int as count FROM push_subscriptions`;
      return r[0]?.count || 0;
    },
    ...
    async getLeads(filters = {}) {
      let q = sql`SELECT l.*, u.name as assignee_name, c.name as creator_name
        FROM leads l
    ...
    async getLead(id) {
      const r = await sql`SELECT l.*, u.name as assignee_name FROM leads l LEFT JOIN users u ON l.assigned_to=u.id WHERE l.id=${id}`;
      return r[0];
    },
    async createLead(data) {
      const r = await sql`INSERT INTO leads(id,name,company,title,industry,company_size,source,profile_url,email,phone,location,bio,recent_post,icp_score,icp_notes,status,priority,assigned_to,notes,drafted_message,tags,created_by)
        VALUES(${data.id},${data.name},${data.company||null},${data.title||null},${data.industry||null},${data.company_size||null},${data.source||'linkedin'},${data.profile_url||null},${data.email||null},${data.phone||null},${data.location||null},${data.bio||null},${data.recent_post||null},${data.icp_score||0},${data.icp_notes||null},${data.status||'new'},${data.priority||'medium'},${data.assigned_to||null},${data.notes||null},${data.drafted_message||null},${data.tags||null},${data.created_by})
        RETURNING *`;
      return r[0];
    },
    async updateLead(id, data) {
      const r = await sql`UPDATE leads SET
        name=COALESCE(${data.name||null},name), company=${data.company||null}, title=${data.title||null},
        industry=${data.industry||null}, company_size=${data.company_size||null},
        profile_url=${data.profile_url||null}, email=${data.email||null}, phone=${data.phone||null},
        location=${data.location||null}, bio=${data.bio||null}, recent_post=${data.recent_post||null},
        icp_score=${data.icp_score!==undefined?data.icp_score:null}, icp_notes=${data.icp_notes||null},
        status=${data.status||null}, priority=${data.priority||null}, assigned_to=${data.assigned_to||null},
        notes=${data.notes||null}, drafted_message=${data.drafted_message||null}, tags=${data.tags||null},
        next_followup=${data.next_followup||null}, last_contacted=${data.last_contacted||null},
        updated_at=NOW()
        WHERE id=${id} RETURNING *`;
      return r[0];
    },
    async deleteLead(id) { await sql`DELETE FROM leads WHERE id=${id}`; },
    async addInteraction({id, lead_id, type, note, created_by}) {
      const r = await sql`INSERT INTO lead_interactions(id,lead_id,type,note,created_by) VALUES(${id},${lead_id},${type},${note||null},${created_by}) RETURNING *`;
      return r[0];
    },
    async getInteractions(lead_id) {
      return sql`SELECT li.*, u.name as creator_name FROM lead_interactions li LEFT JOIN users u ON li.created_by=u.id WHERE li.lead_id=${lead_id} ORDER BY li.created_at DESC`;
    },

    // ── REIMBURSEMENTS ───────────────────────────────
    async getReimbursements(userId, isAdmin) {
      if (isAdmin) return sql`SELECT r.id,r.title,r.category,r.amount,r.description,r.submitted_by,r.status,r.admin_note,r.reviewed_by,r.reviewed_at,r.created_at,u.name as submitter_name FROM reimbursements r LEFT JOIN users u ON r.submitted_by=u.id ORDER BY r.created_at DESC`;
      return sql`SELECT r.id,r.title,r.category,r.amount,r.description,r.submitted_by,r.status,r.admin_note,r.reviewed_by,r.reviewed_at,r.created_at,u.name as submitter_name FROM reimbursements r LEFT JOIN users u ON r.submitted_by=u.id WHERE r.submitted_by=${userId} ORDER BY r.created_at DESC`;
    },
    async createReimbursement({id, title, category, amount, description, receipt_url, submitted_by}) {
      const r = await sql`INSERT INTO reimbursements(id,title,category,amount,description,receipt_url,submitted_by,status) VALUES(${id},${title},${category},${amount},${description||null},${receipt_url||null},${submitted_by},'pending') RETURNING *`;
      return r[0];
    },
    async updateReimbursementStatus(id, status, admin_note, reviewed_by) {
      const r = await sql`UPDATE reimbursements SET status=${status}, admin_note=${admin_note||null}, reviewed_by=${reviewed_by}, reviewed_at=NOW() WHERE id=${id} RETURNING *`;
      return r[0];
    },
    ...
    async getGalleryAlbums() {
      return sql`SELECT ga.*, u.name as creator_name,
        (SELECT COUNT(*)::int FROM gallery_photos gp WHERE gp.album_id=ga.id AND gp.status='approved') as photo_count,
        (SELECT url FROM gallery_photos gp WHERE gp.album_id=ga.id AND gp.status='approved' ORDER BY gp.created_at DESC LIMIT 1) as latest_photo
        FROM gallery_albums ga LEFT JOIN users u ON ga.created_by=u.id ORDER BY ga.created_at DESC`;
    },
    async createGalleryAlbum({id, name, description, created_by}) {
      const r = await sql`INSERT INTO gallery_albums(id,name,description,created_by) VALUES(${id},${name},${description||null},${created_by}) RETURNING *`;
      return r[0];
    },
    async deleteGalleryAlbum(id) {
      await sql`DELETE FROM gallery_albums WHERE id=${id}`;
    },
    async getGalleryPhotos({albumId, status, all} = {}) {
      if (all) return sql`SELECT gp.id,gp.album_id,gp.type,gp.caption,gp.uploaded_by,gp.status,gp.created_at,u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id ORDER BY gp.created_at DESC`;
      if (albumId) return sql`SELECT gp.id,gp.album_id,gp.url,gp.type,gp.caption,gp.status,gp.created_at,u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id WHERE gp.album_id=${albumId} AND gp.status='approved' ORDER BY gp.created_at DESC LIMIT 50`;
      if (status) return sql`SELECT gp.id,gp.album_id,gp.url,gp.type,gp.caption,gp.status,gp.created_at,u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id WHERE gp.status=${status} ORDER BY gp.created_at DESC LIMIT 50`;
      return sql`SELECT gp.id,gp.album_id,gp.url,gp.type,gp.caption,gp.status,gp.created_at,u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id WHERE gp.status='approved' ORDER BY gp.created_at DESC LIMIT 50`;
    },
    async getPendingPhotos() {
      return sql`SELECT gp.id,gp.album_id,gp.url,gp.type,gp.caption,gp.uploaded_by,gp.status,gp.created_at,u.name as uploader_name FROM gallery_photos gp LEFT JOIN users u ON gp.uploaded_by=u.id WHERE gp.status='pending' ORDER BY gp.created_at DESC LIMIT 20`;
    },
    async addGalleryPhoto({id, album_id, url, type, caption, uploaded_by}) {
      const r = await sql`INSERT INTO gallery_photos(id,album_id,url,type,caption,uploaded_by,status) VALUES(${id},${album_id||null},${url},${type||'image'},${caption||null},${uploaded_by},'pending') RETURNING *`;
      return r[0];
    },
    async approvePhoto(id, approved_by) {
      const r = await sql`UPDATE gallery_photos SET status='approved', approved_by=${approved_by}, approved_at=NOW() WHERE id=${id} RETURNING *`;
      return r[0];
    },
    ...
    async updateAlbumCover(albumId) {
      await sql`UPDATE gallery_albums SET cover_photo=(SELECT url FROM gallery_photos WHERE album_id=${albumId} AND status='approved' ORDER BY created_at DESC LIMIT 1) WHERE id=${albumId}`;
    },

    // ── ATTENDANCE ───────────────────────────────────
    async getAttendance({ month, year, userId } = {}) {
      if (userId) return sql`SELECT * FROM attendance WHERE user_id=${userId} ORDER BY date DESC LIMIT 60`;
      if (month && year) {
        const from = `${year}-${String(month).padStart(2,'0')}-01`;
        const to   = `${year}-${String(month).padStart(2,'0')}-31`;
        return sql`SELECT a.*, u.name as user_name, u.image as user_image, u.job_title FROM attendance a LEFT JOIN users u ON a.user_id=u.id WHERE a.date >= ${from} AND a.date <= ${to} ORDER BY a.date DESC, u.name`;
      }
      const today = new Date().toISOString().split('T')[0];
      return sql`SELECT a.*, u.name as user_name, u.image as user_image, u.job_title FROM attendance a LEFT JOIN users u ON a.user_id=u.id WHERE a.date = ${today} ORDER BY u.name`;
    },
    async markAttendance({ id, user_id, date, status, note, marked_by }) {
      const r = await sql`INSERT INTO attendance(id,user_id,date,status,note,marked_by)
        VALUES(${id},${user_id},${date},${status},${note||null},${marked_by})
        ON CONFLICT(user_id,date) DO UPDATE SET status=${status},note=${note||null},marked_by=${marked_by}
        RETURNING *`;
      return r[0];
    },
    ...
    async getAttendanceSummary(userId, year, month) {
      const from = `${year}-${String(month).padStart(2,'0')}-01`;
      const to   = `${year}-${String(month).padStart(2,'0')}-31`;
      const r = await sql`SELECT status, COUNT(*)::int as count FROM attendance WHERE user_id=${userId} AND date >= ${from} AND date <= ${to} GROUP BY status`;
      return r;
    },

    // ── IMPORTANT DAYS ───────────────────────────────
    async getImportantDays() {
      return sql`SELECT * FROM important_days ORDER BY date ASC`;
    },
    async addImportantDay({id, title, date, color, emoji, recurring, created_by}) {
      const r = await sql`INSERT INTO important_days(id,title,date,color,emoji,recurring,created_by) VALUES(${id},${title},${date},${color||'#FF4D6D'},${emoji||'🎉'},${recurring!==false},${created_by||null}) RETURNING *`;
      return r[0];
    },
    async deleteImportantDay(id) {
      await sql`DELETE FROM important_days WHERE id=${id}`;
    },
    ...
    async clearCalendarPosts(calendarId) {
      await sql`DELETE FROM calendar_posts WHERE calendar_id=${calendarId}`;
    },
    async clearCalendarPostsByMonth(calendarId, year, month) {
      // month is 1-based (1=Jan, 12=Dec)
      const from = `${year}-${String(month).padStart(2,'0')}-01`;
      const to   = `${year}-${String(month).padStart(2,'0')}-31`;
      if (calendarId === 'all') {
        await sql`DELETE FROM calendar_posts WHERE publish_date >= ${from} AND publish_date <= ${to}`;
      } else {
        await sql`DELETE FROM calendar_posts WHERE calendar_id=${calendarId} AND publish_date >= ${from} AND publish_date <= ${to}`;
      }
    },
    async clearAllCalendarPosts() {
      // Clear ALL posts from ALL calendars (admin only)
      await sql`DELETE FROM calendar_posts`;
    },
