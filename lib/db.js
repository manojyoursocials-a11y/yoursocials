...
// Get a SQL client — reused across requests in the same lambda instance
function sql() {
...
// ── Schema setup — called from /api/setup, runs once ────────────
export async function setupDatabase() {
  ...
  await db`
    CREATE TABLE IF NOT EXISTS tasks (
      ...
      ai_checklist     TEXT NOT NULL DEFAULT '[]',
      created_by       TEXT,
  ...
  // Seed rewards only if table is empty
...
export function getDb() {
  ...
  return {
    ...
    async createUser({ id, email, name, password, role = 'member', job_title = '' }) {
      const rows = await db`
        INSERT INTO users (id, email, name, password, role, job_title)
        VALUES (${id}, ${email}, ${name}, ${password}, ${role}, ${job_title})
        RETURNING id, email, name, role, job_title, coins, streak
      `;
      return rows[0];
    },
    ...
    async addCoins(userId, amount) {
      await db`UPDATE users SET coins = coins + ${amount} WHERE id = ${userId}`;
    },
    ...
    async createClient({ id, name, contact_name, contact_email, contact_phone, industry, notes, status = 'active', created_by }) {
      const rows = await db`
        INSERT INTO clients (id, name, contact_name, contact_email, contact_phone, industry, notes, status, created_by)
        VALUES (${id}, ${name}, ${contact_name||null}, ${contact_email||null}, ${contact_phone||null}, ${industry||null}, ${notes||null}, ${status}, ${created_by||null})
        RETURNING *
      `;
      return rows[0];
    },
    ...
    async createTask({ id, title, description, priority, owner_id, client_id, deadline, estimated_hours, ai_checklist, created_by }) {
      const rows = await db`
        INSERT INTO tasks (id, title, description, priority, owner_id, client_id, deadline, estimated_hours, ai_checklist, created_by)
        VALUES (${id}, ${title}, ${description||''}, ${priority||'P3'}, ${owner_id||null}, ${client_id||null}, ${deadline||null}, ${estimated_hours||null}, ${ai_checklist||'[]'}, ${created_by||null})
        RETURNING *
      `;
      return rows[0];
    },
    ...
    async updateTask(id, updates) {
      ...
      const fields  = Object.keys(updates).filter(k => allowed.includes(k));
      for (const f of fields) {
        await db`UPDATE tasks SET ${db(f)} = ${updates[f]}, updated_at = NOW() WHERE id = ${id}`;
      }
    ...
    async createFollowup({ id, client_id, subject, due_date, assigned_to, created_by }) {
      const rows = await db`
        INSERT INTO followups (id, client_id, subject, due_date, assigned_to, created_by)
        VALUES (${id}, ${client_id||null}, ${subject}, ${due_date||null}, ${assigned_to||null}, ${created_by||null})
        RETURNING *
      `;
      return rows[0];
    },
    ...
    async updateFollowup(id, updates) {
      const { status, body } = updates;
      await db`
        UPDATE followups SET
          status     = COALESCE(${status||null}, status),
          body       = COALESCE(${body||null},   body),
          sent_at    = CASE WHEN ${status||null} = 'done' THEN NOW() ELSE sent_at END,
          updated_at = NOW()
        WHERE id = ${id}
      `;
    },
    ...
    async deleteFollowup(id) {
      await db`DELETE FROM followups WHERE id = ${id}`;
    },
    ...
    async getMembers() {
      return db`
        SELECT
          u.id, u.email, u.name, u.role, u.job_title, u.coins, u.streak, u.created_at,
    ...
    // ── REWARDS ────────────────────────────────────────────────
    async getRewards() {
      return db`SELECT * FROM rewards ORDER BY coin_cost`;
    },
