...
export async function setupDatabase() {
  ...
  await sql`CREATE TABLE IF NOT EXISTS tasks (id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT NOT NULL DEFAULT '', link TEXT, status TEXT NOT NULL DEFAULT 'todo', priority TEXT NOT NULL DEFAULT 'P3', owner_id TEXT, client_id TEXT, deadline DATE, post_date DATE, estimated_hours NUMERIC(5,1), actual_hours NUMERIC(5,1), quality_rating INTEGER, ai_checklist TEXT NOT NULL DEFAULT '[]', created_by TEXT, completed_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;
...
export function getDb() {
  ...
  return {
    ...
    async getTaskById(id) {
      const r=await sql`SELECT * FROM tasks WHERE id=${id} LIMIT 1`;
      return r[0]||null;
    },
    ...
    async createTask({id,title,description,link,priority,owner_id,client_id,deadline,post_date,estimated_hours,ai_checklist,created_by}) {
      const r=await sql`INSERT INTO tasks(id,title,description,link,priority,owner_id,client_id,deadline,post_date,estimated_hours,ai_checklist,created_by) VALUES(${id},${title},${description||''},${link||null},${priority||'P3'},${owner_id||null},${client_id||null},${deadline||null},${post_date||null},${estimated_hours||null},${ai_checklist||'[]'},${created_by||null}) RETURNING *`;
      return r[0];
    },
    ...
    async editTask(id, f) {
      await sql`UPDATE tasks SET title=${f.title||''},description=${f.description||''},link=${f.link||null},priority=${f.priority||'P3'},owner_id=${f.owner_id||null},client_id=${f.client_id||null},deadline=${f.deadline||null},post_date=${f.post_date||null},estimated_hours=${f.estimated_hours||null},ai_checklist=${f.ai_checklist||'[]'},updated_at=NOW() WHERE id=${id}`;
    },
