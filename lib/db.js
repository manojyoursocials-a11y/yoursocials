...
export function getDb() {
  ...
  return {
    ...
    async createTask({id,title,description,links,media,priority,owner_id,client_id,deadline,post_date,estimated_hours,ai_checklist,created_by}) {
      const r=await sql`INSERT INTO tasks(id,title,description,links,media,priority,owner_id,client_id,deadline,post_date,estimated_hours,ai_checklist,created_by) VALUES(${id},${title},${description||''},${links||'[]'},${media||'[]'},${priority||'P3'},${owner_id||null},${client_id||null},${deadline||null},${post_date||null},${estimated_hours||null},${ai_checklist||'[]'},${created_by||null}) RETURNING *`;
      return r[0];
    },
    ...
    async editTask(id, f) {
      await sql`UPDATE tasks SET title=${f.title||''},description=${f.description||''},links=${f.links||'[]'},media=${f.media||'[]'},priority=${f.priority||'P3'},owner_id=${f.owner_id||null},client_id=${f.client_id||null},deadline=${f.deadline||null},post_date=${f.post_date||null},estimated_hours=${f.estimated_hours||null},ai_checklist=${f.ai_checklist||'[]'},updated_at=NOW() WHERE id=${id}`;
    },
    ...
    async getNewNotifications(userId, since) {
      return sql`SELECT * FROM notifications WHERE user_id=${userId} AND created_at > ${since} ORDER BY created_at DESC`;
    },
