...
function getSql() {
  if (!process.env.DATABASE_URL) throw new Error('DATABASE_URL not set in Vercel environment variables.');
...
export async function setupDatabase() {
  ...
  await sql`CREATE TABLE IF NOT EXISTS tasks (id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT NOT NULL DEFAULT '', link TEXT, status TEXT NOT NULL DEFAULT 'todo', priority TEXT NOT NULL DEFAULT 'P3', owner_id TEXT, client_id TEXT, deadline DATE, estimated_hours NUMERIC(5,1), actual_hours NUMERIC(5,1), quality_rating INTEGER, ai_checklist TEXT NOT NULL DEFAULT '[]', created_by TEXT, completed_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;
  await sql`ALTER TABLE tasks ADD COLUMN IF NOT EXISTS link TEXT`;
  ...
  // NO default rewards seeded — admin creates everything
  return { ok: true };
}
...
export function getDb() {
  ...
  return {
    ...
    // TASKS — separate move and edit, explicit SQL
    async getTasks() {
      return sql`SELECT t.*,u.name AS owner_name,c.name AS client_name FROM tasks t LEFT JOIN users u ON t.owner_id=u.id LEFT JOIN clients c ON t.client_id=c.id ORDER BY t.created_at DESC`;
    },
    async getTasksByMember(memberId) {
      return sql`SELECT t.*,c.name AS client_name FROM tasks t LEFT JOIN clients c ON t.client_id=c.id WHERE t.owner_id=${memberId} ORDER BY t.status,t.created_at DESC`;
    },
    async createTask({id,title,description,link,priority,owner_id,client_id,deadline,estimated_hours,ai_checklist,created_by}) {
      const r=await sql`INSERT INTO tasks(id,title,description,link,priority,owner_id,client_id,deadline,estimated_hours,ai_checklist,created_by) VALUES(${id},${title},${description||''},${link||null},${priority||'P3'},${owner_id||null},${client_id||null},${deadline||null},${estimated_hours||null},${ai_checklist||'[]'},${created_by||null}) RETURNING *`;
      return r[0];
    },
    async moveTask(id, status) {
      if      (status==='done')       await sql`UPDATE tasks SET status='done',       completed_at=NOW(),  updated_at=NOW() WHERE id=${id}`;
      else if (status==='inprogress') await sql`UPDATE tasks SET status='inprogress',                      updated_at=NOW() WHERE id=${id}`;
      else if (status==='review')     await sql`UPDATE tasks SET status='review',                          updated_at=NOW() WHERE id=${id}`;
      else if (status==='todo')       await sql`UPDATE tasks SET status='todo',       completed_at=NULL,   updated_at=NOW() WHERE id=${id}`;
      else throw new Error('Invalid status: '+status);
    },
    async editTask(id, f) {
      await sql`UPDATE tasks SET title=${f.title||''},description=${f.description||''},link=${f.link||null},priority=${f.priority||'P3'},owner_id=${f.owner_id||null},client_id=${f.client_id||null},deadline=${f.deadline||null},estimated_hours=${f.estimated_hours||null},ai_checklist=${f.ai_checklist||'[]'},updated_at=NOW() WHERE id=${id}`;
    },
    ...
    // REWARDS — fully admin controlled, no defaults
    ...
    async updateReward(id,f) {
      await sql`UPDATE rewards SET name=${f.name||'Reward'},emoji=${f.emoji||'🎁'},description=${f.description||''},coin_cost=${f.coin_cost||0},reward_type=${f.reward_type||'weekly'} WHERE id=${id}`;
    },
    ...
    async getDashboard() {
      const [tasks,members,rewards,followups]=await Promise.all([
        sql`SELECT status,priority,deadline::text,quality_rating,title FROM tasks`,
