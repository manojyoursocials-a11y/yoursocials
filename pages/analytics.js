...
export default function Analytics() {
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      Promise.all([api('/api/tasks'), api('/api/members'), api('/api/clients')]).then(([t, m, c]) => {
  ...
  const byClient = clients
    .map(c => ({ name: c.name, count: tasks.filter(t => t.client_id === c.id).length }))
    .filter(c => c.count > 0)
    ...
    <Layout>
      <div className="fade-up">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 14, marginBottom: 20 }}>
          {[
            ['Total Tasks',   tasks.length,                                       'var(--purple)'],
            ['Done',          done,                                                'var(--green)'],
            ['In Progress',   tasks.filter(t => t.status === 'inprogress').length, 'var(--cyan)'],
            ['Overdue',       overdue,                                             overdue > 0 ? 'var(--red)' : 'var(--green)'],
            ['Members',       members.length,                                      'var(--pink)'],
            ['Clients',       clients.length,                                      'var(--orange)'],
          ].map(([l, v, col]) => (
            <Card key={l} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '2rem', fontWeight: 900, color: col }}>{v}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          ...
          <Card>
            ...
            {byClient.length === 0
              ? <div style={{ color: 'var(--muted)', fontSize: '.8rem' }}>No client tasks yet.</div>
              : byClient.map((c, i) => (
                <div key={c.name} style={{ marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.78rem', marginBottom: 4 }}>
                    <span>{c.name}</span><span style={{ color: 'var(--muted2)' }}>{c.count}</span>
                  </div>
                  <ProgressBar value={c.count} max={maxClient} color={MEMBER_COLORS[i % MEMBER_COLORS.length]} />
                </div>
              ))
            }
          </Card>
        ...
        <Card>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
            {[
              ['📋', 'To Do',        tasks.filter(t => t.status === 'todo').length,       'var(--muted2)'],
              ['⚡', 'In Progress',  tasks.filter(t => t.status === 'inprogress').length,  'var(--cyan)'],
              ['👁️', 'Review',       tasks.filter(t => t.status === 'review').length,      'var(--yellow)'],
              ['✅', 'Done',         done,                                                   'var(--green)'],
            ].map(([e, l, v, col]) => (
