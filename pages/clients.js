...
export default function Clients() {
  ...
  async function save() {
    ...
    if (editing) {
      await api('/api/clients', 'PATCH', { id: editing.id, ...form });
      toast.success('Client updated');
    } else {
      await api('/api/clients', 'POST', form);
  ...
  const STATUS_STYLE = {
    active:   { bg: 'rgba(0,229,160,.1)',   color: 'var(--green)', label: 'Active' },
    inactive: { bg: 'rgba(107,107,138,.1)', color: 'var(--muted)', label: 'Inactive' },
    prospect: { bg: 'rgba(0,212,255,.1)',   color: 'var(--cyan)',  label: 'Prospect' },
  };
                  ...
                  <Card key={c.id} hover>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 800, fontSize: '.92rem' }}>{c.name}</div>
