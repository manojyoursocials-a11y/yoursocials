...
import { Card, Btn, Modal, Input, Select, EmptyState, Spinner, toast, sounds, api, askAI } from '../components/UI';
      ...
      <div className="fade-up">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            ...
            <p style={{ fontSize: '.82rem', color: 'var(--muted2)' }}>{pending.length} pending · {done.length} done</p>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        {f.due_date && (
                          <span style={{ background: 'rgba(255,77,109,.1)', color: 'var(--red)', fontSize: '.68rem', fontWeight: 700, padding: '2px 8px', borderRadius: 20 }}>
                            ⏰ {f.due_date}
                          </span>
                        )}
                      </div>
              ...
              {done.length > 0 && (
                <>
                  ...
                  {done.map(f => (
                    <div key={f.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, padding: 12, opacity: .45, display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span>✅</span>
                      <span style={{ fontSize: '.82rem', fontWeight: 600, textDecoration: 'line-through', color: 'var(--muted)' }}>{f.subject}</span>
