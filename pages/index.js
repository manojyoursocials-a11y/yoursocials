      ...
      <div className="fade-up">
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ color: 'var(--muted2)', fontSize: '.88rem' }}>
            {done} tasks done · {overdue > 0 ? `${overdue} overdue ⚠️` : 'no overdue tasks ✅'} · {members.length} team members
          </p>
        </div>
          ...
          {[
            ...
            <Card key={k.label} hover style={{ position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '2rem', fontWeight: 900, color: k.color, lineHeight: 1 }}>
                {k.value}<span style={{ fontSize: '.85rem', fontWeight: 500, color: 'var(--muted)' }}>{k.suffix}</span>
              </div>
            </Card>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          <Card>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
              ...
              <div style={{ flex: 1 }}>
                {[
                  ['Task Completion', completionPct,                        'var(--purple)'],
                  ['On-Time Rate',    Math.max(0, 100 - overdue * 12),     'var(--green)'],
                  ['Team Engaged',    members.length > 0 ? 85 : 0,         'var(--cyan)'],
                ].map(([l, v, col]) => (
                  <div key={l} style={{ marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.73rem', marginBottom: 4 }}>
                      ...
                      <span style={{ fontWeight: 700, color: col }}>{Math.round(v)}%</span>
                    </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
          {[
            ['📋', 'To Do',        todo,   'var(--muted2)'],
            ['⚡', 'In Progress',  inProg, 'var(--cyan)'],
            ['👁️', 'Under Review', review, 'var(--yellow)'],
            ['✅', 'Done',         done,   'var(--green)'],
          ].map(([e, l, v, col]) => (
            ...
            {tripReward
              ? <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.72rem', color: 'var(--muted2)', marginTop: 5 }}>
                    ...
                    <span>Goal: {(tripReward.coin_cost || 13000).toLocaleString()}</span>
                  </div>
                </>
              : <div style={{ color: 'var(--muted)', fontSize: '.82rem' }}>Loading…</div>
            }
