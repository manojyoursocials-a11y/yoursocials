      ...
      <div className="fade-up">
        ...
        <div style={{ display:'grid',gridTemplateColumns:'1fr 280px',gap:16 }}>
          <Card>
            ...
            {members.map((m,i)=>(
              <div key={m.id} style={{ display:'flex',alignItems:'center',gap:12,padding:'10px 12px',borderRadius:10,marginBottom:4,background:i===0?'rgba(255,214,10,.06)':'transparent',border:i===0?'1px solid rgba(255,214,10,.12)':'1px solid transparent',transition:'background .15s' }} onMouseEnter={e=>e.currentTarget.style.background='var(--surface3)'} onMouseLeave={e=>e.currentTarget.style.background=i===0?'rgba(255,214,10,.06)':'transparent'}>
                <div style={{ width:28,textAlign:'center',fontSize:i<3?'1.1rem':'.85rem',fontWeight:700,color:i===0?'var(--yellow)':i===1?'#C0C0C0':i===2?'#CD7F32':'var(--muted)' }}>{i<3?RI[i]:i+1}</div>
          ...
          <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
            ...
            <Card>
              ...
              {[['✅ Task created','+50 🪙'],['✅ Task done','+100 🪙'],['📩 Follow-up sent','+30 🪙']].map(([a,v])=>(
                <div key={a} style={{ display:'flex',justifyContent:'space-between',fontSize:'.75rem',color:'var(--muted2)',padding:'5px 0',borderBottom:'1px solid var(--border)' }}><span>{a}</span><span style={{ fontWeight:700 }}>{v}</span></div>
              ))}
            </Card>
          </div>
        </div>
