  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 16 }}>
      ...
      <div className="scale-in" style={{ background: 'var(--surface2)', border: '1px solid rgba(255,255,255,.13)', borderRadius: 'var(--r-lg)', padding: '40px 36px', width: '100%', maxWidth: 400, textAlign: 'center', position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ fontSize: '.85rem', color: 'var(--muted2)', lineHeight: 1.6, marginBottom: 32 }}>
          The AI-powered operating system for creative teams that move fast and celebrate hard.
        </p>
        <button
          onClick={handleGoogle}
          disabled={loading}
          style={{ width: '100%', padding: '13px 20px', background: 'var(--surface3)', border: '1px solid rgba(255,255,255,.13)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, fontSize: '.9rem', fontWeight: 600, color: 'var(--text)', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'Inter,sans-serif', transition: 'all .18s' }}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, justifyContent: 'center', marginTop: 24 }}>
          {['🔥 Streaks', '🤖 AI Manager', '🏆 Rewards', '✈️ Trip Goal'].map(f => (
            <span key={f} style={{ fontSize: '.7rem', color: 'var(--muted)' }}>{f}</span>
          ))}
        </div>
      </div>
    </div>
