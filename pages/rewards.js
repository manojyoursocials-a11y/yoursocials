...
export default function Rewards() {
  ...
  useEffect(()=>{ if(status==='authenticated') Promise.all([api('/api/rewards'),api('/api/tasks')]).then(([r,t])=>{ setRewards(Array.isArray(r)?r:[]); setTasks(Array.isArray(t)?t:[]); setLoading(false); }); },[status]);
  ...
  return (
    <Layout>
      <div className="fade-up">
        ...
        {trip&&<Card style={{ background:'linear-gradient(135deg,rgba(124,92,252,.12),rgba(0,212,255,.07))',borderColor:'rgba(124,92,252,.25)',marginBottom:20 }}>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:16,flexWrap:'wrap' }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <ProgressBar value={done*50} max={trip.coin_cost||13000} color="linear-gradient(135deg,#00D4FF,#7C5CFC)"/>
              <div style={{ display:'flex',justifyContent:'space-between',fontSize:'.72rem',color:'var(--muted2)',marginTop:5 }}>
                <span>🪙 {Math.min(done*50,trip.coin_cost||13000).toLocaleString()}</span><span>Goal: 🪙{(trip.coin_cost||13000).toLocaleString()}</span>
              </div>
            </div>
          </div>
        </Card>}
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))',gap:14 }}>
          {weekly.map(r=>{
            ...
            return (
              <div key={r.id} onClick={()=>celebrate(r)} style={{ background:unlocked?'rgba(0,229,160,.06)':'var(--surface2)',border:`1px solid ${unlocked?'rgba(0,229,160,.3)':'var(--border)'}`,borderRadius:'var(--r)',padding:20,textAlign:'center',cursor:'pointer',transition:'all .2s',position:'relative',opacity:unlocked?1:.65 }}
              ...
              >
                ...
                <div style={{ fontSize:'.72rem',fontWeight:600,color:unlocked?'var(--green)':'var(--muted2)' }}>{unlocked?'🎉 Tap to celebrate!':'Complete all tasks'}</div>
              </div>
            );
          })}
        </div>
