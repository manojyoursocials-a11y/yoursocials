...
import { Btn, Modal, Input, Select, Textarea, Tag, Avatar, Spinner, launchConfetti, toast, sounds, api, askAI } from '../components/UI';
...
export default function Tasks() {
  ...
  async function moveTask(id, newStatus) {
    await api('/api/tasks', 'PATCH', { id, status: newStatus });
    if (newStatus === 'done') { launchConfetti(); sounds.confetti(); toast.success('Task done! +100 🪙 🎉'); }
    else { sounds.pop(); toast.info('Task moved'); }
    setDetail(null);
    fetchAll();
  }
  ...
  async function genChecklist() {
    ...
    setAiLoad(true);
    const r = await askAI('Generate 5-7 checklist steps for this creative agency task: "' + form.title + '". Return ONLY a numbered list, one item per line.');
  ...
  async function getAISugg(task) {
    setAiSugg('loading');
    const r = await askAI('Give 3 specific suggestions to improve: "' + task.title + '" (Client: ' + (task.client_name || 'Internal') + ', Priority: ' + task.priority + ')');
    setAiSugg(r);
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 110px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
          <select
            ...
            style={{ background: 'var(--surface3)', border: '1px solid var(--border2)', borderRadius: 10, padding: '7px 12px', fontSize: '.8rem', color: 'var(--text)', fontFamily: 'Inter,sans-serif', cursor: 'pointer' }}
          >
            <option value="all">All members</option>
            {members.map(m => <option key={m.id} value={m.id}>{m.name || m.email}</option>)}
          </select>
          <div style={{ marginLeft: 'auto' }}>
            <Btn onClick={() => { setModal(true); setChecklist([]); setForm(df()); }}>+ New Task</Btn>
          </div>
        </div>
            ...
            return (
              <div key={col.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: 8, display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {colTasks.length === 0 && <div style={{ color: 'var(--muted)', fontSize: '.73rem', textAlign: 'center', padding: '20px 0', opacity: .6 }}>No tasks here</div>}
                  {colTasks.map(t => <TaskCard key={t.id} task={t} onClick={() => { setDetail(t); setAiSugg(''); }} />)}
                </div>
                <button
                  onClick={() => { setModal(true); setChecklist([]); setForm(df()); }}
                  style={{ margin: 8, padding: 8, borderRadius: 8, border: '1px dashed var(--border2)', background: 'transparent', color: 'var(--muted)', fontSize: '.74rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, fontFamily: 'Inter,sans-serif', transition: 'all .18s' }}
              ...
              {checklist.map((item, i) => (
                <label key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: '.8rem', cursor: 'pointer', color: 'var(--muted2)', marginBottom: 5 }}>
                  <input
                    type="checkbox"
                    checked={item.done}
                    onChange={e => setChecklist(c => c.map((x, j) => j === i ? { ...x, done: e.target.checked } : x))}
          ...
          {detail && (
            <>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                ...
                {detail.deadline && <span style={{ background: 'var(--surface3)', color: 'var(--muted2)', fontSize: '.68rem', fontWeight: 600, padding: '2px 8px', borderRadius: 6 }}>📅 {detail.deadline}</span>}
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                {COLS.filter(c => c.id !== detail.status).map(col => (
                  <Btn key={col.id} variant="ghost" size="sm" onClick={() => moveTask(detail.id, col.id)}>→ {col.label}</Btn>
                ))}
              </div>
              ...
              {aiSugg === 'loading' && (
                <div style={{ padding: 12, background: 'rgba(124,92,252,.06)', borderRadius: 10, border: '1px solid rgba(124,92,252,.15)', display: 'flex', gap: 4 }}>
              ...
              {aiSugg && aiSugg !== 'loading' && (
                <div style={{ padding: 14, background: 'rgba(124,92,252,.06)', borderRadius: 10, border: '1px solid rgba(124,92,252,.15)', fontSize: '.8rem', color: 'var(--muted2)', lineHeight: 1.65, whiteSpace: 'pre-wrap' }}>🤖 {aiSugg}</div>
              )}
              {detail.ai_checklist && (() => {
                try {
                  ...
                  return (
                    <div style={{ marginTop: 14 }}>
                      <div style={{ fontSize: '.73rem', fontWeight: 700, color: 'var(--muted)', marginBottom: 8, letterSpacing: '.06em', textTransform: 'uppercase' }}>Checklist</div>
                      {cl.map((item, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '.8rem', color: item.done ? 'var(--muted)' : 'var(--muted2)', marginBottom: 5, textDecoration: item.done ? 'line-through' : 'none' }}>
...
function TaskCard({ task, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      style={{ background: 'var(--surface3)', border: '1px solid ' + (overdue ? 'rgba(255,77,109,.3)' : 'var(--border)'), borderRadius: 10, padding: 12, cursor: 'pointer', transition: 'all .18s' }}
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        {task.owner_name && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '.68rem', color: 'var(--muted2)' }}>
            <Avatar name={task.owner_name} url={task.owner_image} size={18} color="var(--purple)" />
            {task.owner_name.split(' ')[0]}
          </div>
        )}
        {task.deadline && (
          <span style={{ fontSize: '.67rem', color: overdue ? 'var(--red)' : 'var(--muted)', fontWeight: overdue ? 700 : 400 }}>
