      ...
      <div className="fade-up">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            ...
            <p style={{ fontSize: '.82rem', color: 'var(--muted2)' }}>{members.length} members · everyone logs in with Google</p>
          </div>
          <Btn variant="ghost" onClick={() => {
            setForm({ job_title: session?.user?.job_title || '', role: session?.user?.role || 'member' });
            setModal(true);
          }}>
            ✏️ Edit My Profile
          </Btn>
              ...
              {members.map((m, i) => {
                ...
                return (
                  <Card key={m.id} hover style={{ border: isMe ? '1px solid rgba(124,92,252,.35)' : undefined, position: 'relative' }}>
                    {isMe && (
                      <div style={{ position: 'absolute', top: 12, right: 12, fontSize: '.63rem', fontWeight: 700, background: 'rgba(124,92,252,.15)', color: 'var(--purple2)', padding: '2px 8px', borderRadius: 20 }}>
                        You
                      </div>
                    )}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                      <div style={{ position: 'relative' }}>
                        <Avatar name={m.name || m.email} url={m.image} size={48} color={color} />
                        {m.streak >= 3 && (
                          <div style={{ position: 'absolute', bottom: -4, right: -4, background: 'var(--orange)', color: '#000', fontSize: '.58rem', fontWeight: 800, padding: '1px 4px', borderRadius: 6 }}>
                            🔥{m.streak}
                          </div>
                        )}
                      </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6, marginBottom: 12 }}>
                      {[
                        ['Done',       done,              color],
                        ['Deadline%',  dlPct + '%',       dlPct >= 90 ? 'var(--green)' : 'var(--yellow)'],
                        ['Quality',    qPct ? qPct + '%' : '—', 'var(--cyan)'],
                      ].map(([l, v, c]) => (
